...
function DownloadCollectionView() {
    ...
    const DownloadCollection = async () => {
        ...
        if (!version || version.trim().length === 0 || selectedLoaders.length === 0 || !directory || directory.trim().length === 0) {
        ...
        try {
            ...
            const result = await (window as any).backend.DownloadCollection(collectionId.trim(), version, selectedLoaders, directory, updateExisting) as CollectionDownloadInfo
            showMessage(
    ...
    const GetModsInCollection = async () => {
        try {
            ...
            if (!version || version.trim().length === 0 || selectedLoaders.length === 0) {
                showMessage(t('sections.collection.fields.incomplete_fields'), { showClose: true })
                return
            }

            showMessage(t('sections.collection.verify.listing'), { showClose: false })
            const result = await (window as any).backend.GetModsInCollectionInfo(collectionId, version, selectedLoaders) as ModsInCollectionInfo
            ...
            <section className="content">
                ...
                <div className="row">
                    <div className="field">
                        ...
                        <MCInput style={{marginTop:  'auto'}} value={version} onChange={(e) => setVersion(e.target.value)} placeholder={t('sections.collection.fields.versionPlaceholder')} />
                    </div>
                    <div className="field">
                        ...
                        <div className="loader-chips">
                            {selectedLoaders.map((l) => (
                                <span key={l} className="loader-chip">
                                    <span>{l}</span>
                                    <button onClick={() => removeLoader(l)}>✕</button>
                                </span>
                            ))}
                        </div>
                        <div style={{ display: 'flex', gap: 8 }}>
                            ...
                            <MCButton onClick={addLoader}>{t('actions.add', { ns: 'commons' })}</MCButton>
                        </div>
                    </div>
                </div>
