...
def install_mrpack_clientside(path: str | PathLike, minecraft_directory: str | PathLike, modpack_directory: str | PathLike | None = None, callback: CallbackDict | None = None, mrpack_install_options: MrpackInstallOptions | None = None) -> None:
    ...
    with zipfile.ZipFile(path, "r") as zf:
        ...
        for count, file in enumerate(file_list):
            ...
            # Verify if the file is a client file
...
def install_mrpack_serverside(path: str | PathLike, minecraft_directory: str | PathLike, modpack_directory: str | PathLike | None = None, callback: CallbackDict | None = None, mrpack_install_options: MrpackInstallOptions | None = None) -> None:
    ...
    with zipfile.ZipFile(path, "r") as zf:
        ...
        for count, file in enumerate(file_list):
            ...
            if file["env"]["server"] == "unsupported":
                # Client only file
                continue
            if file["path"].startswith("resourcepacks/") or file["path"].startswith("shaderpacks/"):
                # Resourcepack or shaderpack, that are not needed for the server
                continue
...
def add_vanilla_launcher_profile(minecraft_directory: str | PathLike, vanilla_profile: NewVanillaLauncherProfile) -> None:
    """
